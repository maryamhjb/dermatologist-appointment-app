...
type DashboardClientProps = {
  /** From server (cookie session); client fetch can lag right after OTP login */
  initialIsAdmin: boolean
}

export function DashboardClient({ initialIsAdmin }: DashboardClientProps) {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          fromApi = Boolean(body.isAdmin)
        }
      } catch {
        /* ignore */
      }

      setIsAdmin(fromApi || initialIsAdmin)
    ...
    checkUser()
    // createClient() is not stable; run once on mount + when server-provided admin flag changes
    // eslint-disable-next-line react-hooks/exhaustive-deps -- supabase client recreated each render
  }, [initialIsAdmin, router])
    ...
    return (
      <div className="min-h-dvh bg-background text-start">
        ...
        <div className="mx-auto max-w-4xl px-4 py-6">
          <p className="text-start">درحال بارگذاری...</p>
          {initialIsAdmin ? (
            <div className="mt-5">
              <Link
    ...
    <div className="min-h-dvh bg-background text-start">
      ...
      <div className="mx-auto max-w-4xl px-4 py-6">
        <div className="mb-8">
          ...
          <p className="mt-1.5 text-sm text-muted-foreground sm:text-[0.9375rem]">رزرو نوبت آنلاین</p>
          {isAdmin ? (
            <div className="mt-5">
              <Link
                href="/admin/dashboard"
            ...
            <Card>
              ...
              <CardContent>
                {appointments.length === 0 ? (
                  <div className="py-8 text-start text-sm text-muted-foreground">نوبتی ثبت نشده است.</div>
                ) : (
                  <div className="space-y-4">
                    {appointments.map((apt) => (
                      <Card key={apt.id} className="border p-4">
                        <div className="flex items-start justify-between">
                          <div>
                            <p className="font-semibold">{apt.office?.name}</p>
                            <p className="text-sm text-muted-foreground">{apt.procedure?.name_fa}</p>
                            <p className="mt-2 text-sm">تاریخ: {apt.time_slot?.slot_date}</p>
                            <p className="text-sm">ساعت: {apt.time_slot?.start_time}</p>
                          </div>
          ...
          <TabsContent value="booking">
            <Card>
              ...
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">با دکمه زیر فرم رزرو باز می‌شود.</p>
